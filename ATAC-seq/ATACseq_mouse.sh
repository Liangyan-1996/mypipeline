...
if [[ -e raw/${sample}_1.fastq.gz ]] && [[ -e raw/${sample}_2.fastq.gz ]]; then
    ...
    $fastp -i raw/${sample}_1.fastq.gz -I raw/${sample}_2.fastq.gz -o qc/${sample}.fastp1.fq.gz -O qc/${sample}.fastp2.fq.gz -w ${thread} 2> $workdir/log/${sample}.log
    $bowtie2 \
        --end-to-end --very-sensitive -x $bowtie2index -p ${thread} \
        -X 2000 --no-mixed --no-discordant --no-unal \
        -1 qc/${sample}.fastp1.fq.gz -2 qc/${sample}.fastp2.fq.gz 2>> $workdir/log/${sample}.log |\
    ...
    $fastp -i raw/${sample}.fastq.gz -o qc/${sample}.fastp.fq.gz -w ${thread} -A 2> $workdir/log/${sample}.log
    $bowtie2 \
        --end-to-end --very-sensitive -x $bowtie2index -p ${thread} \
...
if [[ "$library" == "2" ]];then
    ...
    # final sorting
    $sambamba sort -t $thread -m 10G -o $sample.clean.bam $sample.rmblack.bam
    # length distribution
    $samtools view $sample.clean.bam |\
        awk -F'\t' 'function abs(x){return ((x < 0.0) ? -x : x)} {print $1"\t"abs($9)}' |\
        sort --parallel $thread --unique | cut -f2 > ${sample}.length.txt
...
# summary
cd $workdir
sh $ScriptDir/ATACseq/ATAC-seq_summarize.sh
