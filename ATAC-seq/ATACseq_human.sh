...
ScriptDir="/public/home/liunangroup/liangyan/pipeline/ATACseq/script"
...
$sambamba markdup \
    -r -t ${thread} ${sample}.rmMT.bam ${sample}.dedup.bam 2>> $workdir/log/${sample}.log
$bedtools intersect \
    -nonamecheck -v -a ${sample}.dedup.bam -b $blacklist > ${sample}.rmblack.bam
$samtools index -@ $thread ${sample}.rmblack.bam
$alignmentSieve \
    --numberOfProcessors $thread --ATACshift -b $sample.rmblack.bam -o $sample.shift.bam
$sambamba sort -t $thread -m 10G -o $sample.clean.bam $sample.shift.bam

$Rscript $ScriptDir/TSS_enrich_score.R ${sample}.clean.bam ${ucsc_anno} >> $workdir/log/${sample}.log
...
$computeMatrix reference-point \
    -S ${sample}.bw -R $TSS -a 3000 -b 3000 -p ${thread} -o ${sample}.gz
$plotHeatmap \
    -m ${sample}.gz -out ${sample}.png --dpi 300 --plotFileFormat png --yMin 0
